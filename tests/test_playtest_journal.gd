...
func _init() -> void:
	...
	journal.record("road_event_reached", {"event": "salvage_choice", "origin": "ashgate_depot", "destination": "soot_orchard"})
	journal.record("road_event_resolved", {"event": "salvage_choice", "choice": "take_fuel"})
	journal.record("road_arrival_completed", {"origin": "ashgate_depot", "destination": "soot_orchard"})
	...
	_expect(int(feedback_data.get("schema_version", 0)) == 2, "feedback should identify the journey-continuity schema")
	...
	_expect(int(metrics.get("journey_commitments", 0)) == 1 and int(metrics.get("road_events_reached", 0)) == 1 and int(metrics.get("road_events_resolved", 0)) == 1 and int(metrics.get("road_arrivals_completed", 0)) == 1, "feedback should summarize the local route-to-arrival sequence without inferring comprehension")
