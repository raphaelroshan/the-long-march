...
func _run_plan(accept_contract: bool, public_beacons: bool) -> void:
	...
	_expect(state.phase == "settlement" and state.settlement_actions_remaining == (2 if accept_contract else 1), "Windbreak should expose contract-shaped recovery capacity")
