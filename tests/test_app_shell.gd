...
func _run() -> void:
	...
	_expect(app.start_button.get_node_or_null(app.start_button.focus_neighbor_bottom) == app.veyru_start_button and app.veyru_start_button.get_node_or_null(app.veyru_start_button.focus_neighbor_bottom) == app.cinder_start_button and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_neighbor_bottom) == app.settings_button, "no-save navigation should include all playable regions while routing around hidden replay actions")
	_expect(app.start_button.get_node_or_null(app.start_button.focus_next) == app.veyru_start_button and app.veyru_start_button.get_node_or_null(app.veyru_start_button.focus_next) == app.cinder_start_button and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_next) == app.guide_button and app.quit_button.get_node_or_null(app.quit_button.focus_next) == app.tutorial_button, "no-save Tab navigation should include the tutorial and all regions while skipping Continue")
	...
	_expect(app.title_charter_label.text.contains("MARCH CHARTER · 0/3 REGIONS SURVIVED") and app.title_charter_label.text.contains("Choose any chapter"), "a fresh title should present the bounded three-chapter charter without inventing progress")
	...
	_expect(app.save_recovery_button.visible and app.save_recovery_button.text == "REMOVE UNUSABLE SAVE" and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_neighbor_bottom) == app.save_recovery_button, "an invalid save should expose an accurately named recovery action after all new-run choices")
	_expect(not app.quick_start_button.visible and app.tutorial_button.get_node_or_null(app.tutorial_button.focus_next) == app.start_button and app.start_button.get_node_or_null(app.start_button.focus_next) == app.veyru_start_button and app.veyru_start_button.get_node_or_null(app.veyru_start_button.focus_next) == app.cinder_start_button and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_next) == app.save_recovery_button, "invalid-save navigation should retain the tutorial, all regions, and recovery while collapsing the replay shortcut")
	app._continue_game()
	await process_frame
	...
	app.settings_close_button.pressed.emit()
	await process_frame
	_expect(app.title_charter_label.text.contains("MARCH CHARTER · 0/3 REGIONS SURVIVED") and not app.veyru_start_button.tooltip_text.contains("Public Archive Signal is active"), "the title should immediately return to a clean Charter and remove development-specific guidance")
	...
	_expect(app.campaign_progress.developments.is_empty() and app.campaign_progress.region_results.is_empty() and app.title_charter_label.text.contains("0/3 REGIONS SURVIVED"), "clean-start reset should rebuild an empty in-memory March Charter")
