...
func _run() -> void:
	...
	_expect(app.start_button.get_node_or_null(app.start_button.focus_neighbor_bottom) == app.veyru_start_button and app.veyru_start_button.get_node_or_null(app.veyru_start_button.focus_neighbor_bottom) == app.cinder_start_button and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_neighbor_bottom) == app.salt_start_button and app.salt_start_button.get_node_or_null(app.salt_start_button.focus_neighbor_bottom) == app.settings_button, "no-save navigation should include all playable regions while routing around hidden replay actions")
	_expect(app.start_button.get_node_or_null(app.start_button.focus_next) == app.veyru_start_button and app.veyru_start_button.get_node_or_null(app.veyru_start_button.focus_next) == app.cinder_start_button and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_next) == app.salt_start_button and app.salt_start_button.get_node_or_null(app.salt_start_button.focus_next) == app.guide_button and app.quit_button.get_node_or_null(app.quit_button.focus_next) == app.tutorial_button, "no-save Tab navigation should include the tutorial and all regions while skipping Continue")
	...
	_expect(app.title_charter_label.text.contains("MARCH CHARTER · 0/4 REGIONS SURVIVED") and app.title_charter_label.text.contains("Choose any chapter"), "a fresh title should present the bounded four-chapter charter without inventing progress")
	...
	_expect(app.save_recovery_button.visible and app.save_recovery_button.text == "REMOVE UNUSABLE SAVE" and app.salt_start_button.get_node_or_null(app.salt_start_button.focus_neighbor_bottom) == app.save_recovery_button, "an invalid save should expose an accurately named recovery action after all new-run choices")
	_expect(not app.quick_start_button.visible and app.tutorial_button.get_node_or_null(app.tutorial_button.focus_next) == app.start_button and app.start_button.get_node_or_null(app.start_button.focus_next) == app.veyru_start_button and app.veyru_start_button.get_node_or_null(app.veyru_start_button.focus_next) == app.cinder_start_button and app.cinder_start_button.get_node_or_null(app.cinder_start_button.focus_next) == app.salt_start_button and app.salt_start_button.get_node_or_null(app.salt_start_button.focus_next) == app.save_recovery_button, "invalid-save navigation should retain the tutorial, all regions, and recovery while collapsing the replay shortcut")
	app._continue_game()
	await process_frame
	_expect(app.save_recovery_button.has_focus(), "a failed Continue attempt should focus the newly available recovery action")
	_expect(app.title_preview_id == "recovery" and app.title_preview_title_label.text == "Remove the unusable checkpoint" and app.title_region_briefing_label.text.contains("cannot be loaded"), "focusing save recovery should explain the unusable primary and keep both new journeys available")
	...
	_expect(_tree_contains_text(app.guide_view, "Known roads name contacts and counters") and _tree_contains_text(app.guide_view, "Regional pressure can close one road") and _tree_contains_text(app.guide_view, "always preserves a recovery path") and _tree_contains_text(app.guide_view, "only one emergency order"), "the field guide should explain regional visibility, closure, recovery, and intervention rules, not only list screens")
	_expect(_tree_contains_text(app.guide_view, "all chapters keep the normal simulation, seed, route graph, and checkpoint rules"), "the chapter-start note should preserve normal save expectations instead of claiming that the save file cannot change")
	...
	_expect(app.game_view.reduced_motion_enabled and app.game_view.settlement_hub.reduced_motion_enabled and app.game_view.settlement_hub.bazaar_canvas.reduced_motion and app.game_view.journey_transition.reduced_motion and app.game_view.journey_transition.march_canvas.reduced_motion and app.game_view.road_contact.reduced_motion and app.game_view.road_contact.contact_canvas.reduced_motion, "a newly opened playable stage should apply the persisted reduced-motion preference to settlement, road, and contact animation")
	...
	await process_frame
	_expect(app.title_preview_id == "veyru" and app.title_region_briefing_label.text.contains("Public Archive Signal is active") and app.title_region_briefing_label.text.contains("Drowned Registry contacts begin Known") and app.veyru_start_button.tooltip_text.contains("Public Archive Signal is active"), "the focused Veyru preview should explain the unlocked regional development before the next run")
	_expect(app.title_charter_label.text.contains("MARCH CHARTER · 1/4 REGIONS SURVIVED") and app.title_charter_label.text.contains("Veyru Archive Scarred") and app.title_charter_label.text.contains("Next: Ashgate Lowlands"), "the title Charter should retain the best Veyru outcome and direct the player toward the first unfinished chapter")
	...
	app.settings_close_button.pressed.emit()
	await process_frame
	_expect(app.title_charter_label.text.contains("MARCH CHARTER · 0/4 REGIONS SURVIVED") and not app.veyru_start_button.tooltip_text.contains("Public Archive Signal is active"), "the title should immediately return to a clean Charter and remove development-specific guidance")
	...
	_expect(app.campaign_progress.developments.is_empty() and app.campaign_progress.region_results.is_empty() and app.title_charter_label.text.contains("0/4 REGIONS SURVIVED"), "clean-start reset should rebuild an empty in-memory March Charter")
