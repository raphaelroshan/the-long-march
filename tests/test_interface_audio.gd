...
func _run() -> void:
	...
	_expect(audio.play_semantic("contact_impact") and audio.last_semantic_cue_kind == "contact_impact" and audio.last_semantic_asset_path.ends_with("metalPot3.ogg"), "the resolved impact phase should have a dedicated material cue")
