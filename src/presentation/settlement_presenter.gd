...
static func build(state: LongMarchState, snapshot: Dictionary, fortress: Dictionary) -> Dictionary:
	...
	var contract_status := state.veyru_contract_status if is_veyru else (state.cinder_contract_status if is_cinder else state.guard_contract_status)
	var location_name := String(LongMarchState.JOURNEY_NODES.get(state.current_location, {}).get("name", state.current_location))
	var contract_name := "SEALED MEDICINE DELIVERY" if is_veyru else ("CINDER GUILD DYNAMO" if is_cinder else "MORROWLINE CONVOY GUARD")
	...
	var assignment_station := {
		...
		"body": assignment_body if contract_status == "offered" else ("The fortress accepted this assignment. Its consequences now travel with the march." if contract_status == "accepted" else ("The fortress declined this assignment. Morrowline will have only 1 service action because its parts convoy is absent." if not is_veyru and not is_cinder else "The fortress declined this assignment. The first roads are open without its obligation.")),
	...
	if contract_status == "offered":
		...
		assignment_station["secondary"] = {"id": "decline_assignment", "label": "DECLINE · TRAVEL UNBOUND", "enabled": true, "tooltip": "Decline without spending fuel or time. Morrowline will have only 1 service action." if not is_veyru and not is_cinder else "Decline without spending fuel or time."}
	...
	var settlement_context := "%s · Choose an assignment or inspect a bazaar station." % ("LANTERN QUAY FLOOD MARKET" if is_veyru else ("BLACKKILN FORGE BAZAAR" if is_cinder else "ASHGATE RAIL DEPOT"))
	if contract_status == "accepted":
		settlement_context = "ASSIGNMENT RECEIPT · %s accepted. Prepare the fortress, then plan the first road." % ("Sealed medicine delivery" if is_veyru else ("Cinder Guild dynamo" if is_cinder else "Morrowline convoy guard"))
	elif contract_status == "declined":
		settlement_context = "ASSIGNMENT RECEIPT · Traveling without the local obligation. Prepare the fortress, then plan the first road." if is_veyru or is_cinder else "ASSIGNMENT RECEIPT · Traveling without the convoy. Morrowline will have 1 service action; prepare the fortress, then plan the first road."
	var signal_station := {"title": "Signal Broker", "status": "NO LOCAL REPORTS", "button_status": "QUIET", "body": "Lantern keepers compare water levels and archive signals. Exact forecasts still depend on working signal equipment." if is_veyru else ("Smoke readers compare the fireline against grade beacons. Exact forecasts still depend on working signal equipment." if is_cinder else "Depot signalers compare blockade sightings and ash fronts. Exact forecasts still depend on working signal equipment."), "tone": "muted"}
	if not is_veyru and not is_cinder:
	...
	return {
		...
		"place_identity": "FLOODLINE MARKET · Dry gantries, water gauges, and archive lanterns." if is_veyru else ("VOLCANIC FORGE MARKET · Kiln stacks, grade rails, and lift bells." if is_cinder else "LOWLAND RAILHEAD · Repair yards, black rails, and blockade signals."),
		"operational_pressure": "%s %d · %s" % [state.campaign_pressure_name().to_upper(), state.campaign_pressure, "Rising water can close the exposed registry road." if is_veyru else ("The moving fireline can close Slag Tunnel while opening the Ash Chapel refuge." if is_cinder else "Rising pursuit can close the exposed signal road.")],
		"route_meaning": "Pump Gallery buys control with time; Sunken Tramworks saves time by exposing the lower hull." if is_veyru else ("Charcoal Monastery offers shelter and a vow; Red Cut is faster but punishes heavy chassis." if is_cinder else "Rill Crossing is the direct convoy road; Soot Orchard spends time for salvage and weather exposure."),
