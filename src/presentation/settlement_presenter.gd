...
static func build(state: LongMarchState, snapshot: Dictionary, fortress: Dictionary) -> Dictionary:
	...
	var contract_status := state.veyru_contract_status if is_veyru else (state.cinder_contract_status if is_cinder else (state.salt_contract_status if is_salt else state.guard_contract_status))
	var location_name := String(LongMarchState.JOURNEY_NODES.get(state.current_location, {}).get("name", state.current_location))
	var contract_name := "SEALED MEDICINE DELIVERY" if is_veyru else ("CINDER GUILD DYNAMO" if is_cinder else ("COMPACT BEACON ESCORT" if is_salt else "MORROWLINE CONVOY GUARD"))
	...
	if is_veyru:
		...
		assignment_body = "Carry Blackkiln's dynamo pattern to Switchback Commune. The load adds 1 heat on every road and binds the promise to the Generator Core; delivery pays 30 Ashmarks, 2 trust, and one extra Old Lift service action."
		assignment_accept_enabled = state.operational("generator_core")
	elif is_salt:
		assignment_body = "Guide the Refugee Compact under public beacons. A visible signal draws tougher rival scouts; arrival pays 26 Ashmarks, 2 trust, and one extra Windbreak service."
		assignment_accept_enabled = state.operational("signal_coil") or state.operational("signal_mast")
	else:
	...
	var assignment_station := {
		...
		"body": assignment_body if contract_status == "offered" else ("The fortress accepted this assignment. Its consequences now travel with the march." if contract_status == "accepted" else ("The fortress declined this assignment. Morrowline will have only 1 service action because its parts convoy is absent." if not is_veyru and not is_cinder and not is_salt else "The fortress declined this assignment. The first roads are open without its obligation.")),
	...
	if contract_status == "offered":
		...
		assignment_station["secondary"] = {"id": "decline_assignment", "label": "DECLINE · TRAVEL UNBOUND", "enabled": true, "tooltip": "Decline without spending fuel or time. Morrowline will have only 1 service action." if not is_veyru and not is_cinder and not is_salt else "Decline without spending fuel or time."}
	elif not is_veyru and not is_cinder and not is_salt:
	...
	if contract_status == "accepted":
		settlement_context = "ASSIGNMENT RECEIPT · %s accepted. Prepare the fortress, then plan the first road." % ("Sealed medicine delivery" if is_veyru else ("Cinder Guild dynamo" if is_cinder else ("Compact beacon escort" if is_salt else "Morrowline convoy guard")))
	elif contract_status == "declined":
		settlement_context = "ASSIGNMENT RECEIPT · Traveling without the local obligation. Prepare the fortress, then plan the first road." if is_veyru or is_cinder or is_salt else "ASSIGNMENT RECEIPT · Traveling without the convoy. Morrowline will have 1 service action; prepare the fortress, then plan the first road."
	var signal_station := {"title": "Signal Broker", "status": "NO LOCAL REPORTS", "button_status": "QUIET", "body": "Lantern keepers compare water levels and archive signals. Exact forecasts still depend on working signal equipment." if is_veyru else ("Smoke readers compare the fireline against grade beacons. Exact forecasts still depend on working signal equipment." if is_cinder else ("Saltglass readers compare escort beacons, ash-front bearings, and rival silhouettes across the open flat." if is_salt else "Depot signalers compare blockade sightings and ash fronts. Exact forecasts still depend on working signal equipment.")), "tone": "muted"}
	...
	var quartermaster_station := {"title": "Quartermaster Stores", "status": "%d ASHMARKS · %d FUEL" % [state.money, state.fuel], "button_status": "STORES", "body": "Review medicine space, fuel, and carried modules before the flood roads. No fixed trade is posted at this quay." if is_veyru else ("Review heat margin, fuel, and chassis mass before the climb. Blackkiln posts no fixed trade today." if is_cinder else "Review fuel, parts, and carried modules before leaving the rail depot."), "tone": "neutral", "primary": {"id": "review_supplies", "label": "REVIEW FORTRESS STORES", "enabled": true, "tooltip": "Open the detailed module and capacity view."}}
	if not is_veyru and not is_cinder and not is_salt:
	...
	return {
		...
		"place_identity": "FLOODLINE MARKET · Dry gantries, water gauges, and archive lanterns." if is_veyru else ("VOLCANIC FORGE MARKET · Kiln stacks, grade rails, and lift bells." if is_cinder else ("SALTGLASS SIGNAL MARKET · Wind vanes, water casks, and mirrored beacons." if is_salt else "LOWLAND RAILHEAD · Repair yards, black rails, and blockade signals.")),
		"operational_pressure": "%s %d · %s" % [state.campaign_pressure_name().to_upper(), state.campaign_pressure, "Rising water can close the exposed registry road." if is_veyru else ("The moving fireline can close Slag Tunnel while opening the Ash Chapel refuge." if is_cinder else ("The ash-front whiteout can close Empty Mile while opening Lee Trench." if is_salt else "Rising pursuit can close the exposed signal road."))],
		"route_meaning": "Pump Gallery buys control with time; Sunken Tramworks saves time by exposing the lower hull." if is_veyru else ("Charcoal Monastery offers shelter and a vow; Red Cut is faster but punishes heavy chassis." if is_cinder else ("Buried Observatory improves public bearings; Quiet Caravan trades fuel for convoy cover." if is_salt else "Rill Crossing is the direct convoy road; Soot Orchard spends time for salvage and weather exposure.")),
		...
		"stations": {
			"workshop": {"title": "Chassis Workshop", "status": "REFIT AVAILABLE", "button_status": "REFIT", "body": ("Use the quay's dry gantry to inspect the walking fortress and protect its lower hull before the archive road." if is_veyru else ("Use Blackkiln's grade cradle to inspect mass, heat, and the Generator Core before the climb." if is_cinder else "Use the depot's rail-side repair bay to inspect the walking fortress, trace dependencies, and prepare its movement chain.")), "tone": "safe", "primary": {"id": "open_workshop", "label": "ENTER WORKSHOP", "enabled": true, "tooltip": "Open the detailed chassis workbench."}},
			...
			"departure_gate": {"title": "Departure Gate", "status": "ROUTES READY" if departure_ready else "ASSIGNMENT BLOCKS DEPARTURE", "button_status": "PLAN JOURNEY" if departure_ready else "LOCKED", "body": (("Pump Gallery is the slower managed-water road; Sunken Tramworks is the shorter submerged cut. Open the route table to compare exact costs and intelligence before Commit." if is_veyru else ("Charcoal Monastery is the sheltered vow road; Red Cut is the exposed heavy-grade climb. Open the route table before Commit." if is_cinder else ("Buried Observatory is the public signal road; Quiet Caravan is the faster water-bearing convoy. Open the route table before Commit." if is_salt else "Rill Crossing is the direct convoy road; Soot Orchard is the longer salvage road. Open the route table to compare exact costs and intelligence before Commit."))) if departure_ready else "The settlement requires an answer at the assignment board before it will clear the fortress to leave."), "tone": "safe" if departure_ready else "warning", "primary": {"id": "plan_journey", "label": "PLAN JOURNEY", "enabled": departure_ready, "tooltip": "Open the regional map without committing a route."}}
		}
	}
