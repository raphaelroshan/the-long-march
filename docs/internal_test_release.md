...
22. Open **Build & Local Data** from title Settings and paused Settings. Confirm the exact version/platform, offline statement, local-file presence, export count, and absolute data path are visible. Copy the folder path, then cancel back; a paused run must remain suspended and no external application should open.
