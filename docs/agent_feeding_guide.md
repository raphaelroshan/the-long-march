...
The original prompts below describe the build-up of the prototype and are retained as historical context. The current implementation baseline is `0.3.0-alpha.361`; Feeds A through AT, **LM-EA-1** through **LM-EA-6**, and **LM-I1** through **LM-I5** are complete. Four five-contact chapters, three physical chassis templates, twenty modules, six specialists, causal obligation memory, composable endings, verified Windows/macOS/Linux release cohorts, and deterministic Ashgate/Cinder/White Salt evaluation journeys meet repository automation gates. The next implementation gate is **LM-I6**, hardening the complete Early Access package and recovery boundary. Consented, uncoached human validation then uses [`early_access_test_matrix.md`](early_access_test_matrix.md) and [`private_alpha_session_sheet.md`](private_alpha_session_sheet.md) for calibration, not as a claim that the game is commercially finished.
