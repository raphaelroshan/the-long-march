...
The original prompts below describe the build-up of the prototype and are retained as historical context. The current implementation baseline is `0.3.0-alpha.357`; Feeds A through AT, **LM-EA-1** through **LM-EA-6**, and the **LM-I1 creative vertical lock** are complete. Four five-contact chapters, three physical chassis templates, twenty modules, six specialists, regional memory, composable endings, verified Windows/macOS/Linux release cohorts, and a deterministic investment-evaluation journey meet repository automation gates. The next work is consented, uncoached human validation using [`early_access_test_matrix.md`](early_access_test_matrix.md) and [`private_alpha_session_sheet.md`](private_alpha_session_sheet.md), followed by narrow changes grounded in repeated observations. This is calibration, not a claim that the game is commercially finished.
