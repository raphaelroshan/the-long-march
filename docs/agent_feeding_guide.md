...
Before issuing any implementation feed, direct the agent to read `docs/qa_playbook.md`, `docs/agent_qa_decision.md`, and `AGENTS.md`. Preserve separate baseline and final outputs and classify each result as `PASS`, `FAIL`, `BLOCKED_ENVIRONMENT`, `TIMEOUT_PARTIAL`, or `INVALID_EVIDENCE`. New journey work must use the executable `qa/scenarios/long_march_complete_journey.json` contract and must not claim completion until its exact command trace, route, contact, consequence, recovery, arrival, checkpoints, images, and terminal Debrief validate.
