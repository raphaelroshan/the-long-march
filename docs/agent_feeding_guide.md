...
The original prompts below describe the build-up of the prototype and are retained as historical context. The current implementation baseline is `0.3.0-alpha.359`; Feeds A through AT, **LM-EA-1** through **LM-EA-6**, and **LM-I1** through **LM-I3** are complete. Four five-contact chapters, three physical chassis templates, twenty modules, six specialists, regional memory, composable endings, verified Windows/macOS/Linux release cohorts, and deterministic Ashgate/Cinder/White Salt evaluation journeys meet repository automation gates. The next implementation gate is **LM-I4**, proving the expanded module, specialist, and threat roster creates meaningful dependency choices through complete journeys. Consented, uncoached human validation then uses [`early_access_test_matrix.md`](early_access_test_matrix.md) and [`private_alpha_session_sheet.md`](private_alpha_session_sheet.md) for calibration, not as a claim that the game is commercially finished.
