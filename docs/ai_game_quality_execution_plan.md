...
**Applies to:** `0.3.0-alpha.330` and later
...
| **Live counter readiness — complete in alpha.318** | Turn threat advice into a current-fortress answer | Name operational, offline, or missing module counters in the live contact dossier while preserving deterministic combat. |
| **Threat risk preview — complete in alpha.319** | Make the cost of inaction legible before impact | State each threat family's practical consequence beside its target, counter, and current readiness. |
...
| **Target-lock focus safety — complete in alpha.324** | Prevent accidental blind resolution | Keep Advance focused during forecast, then move focus to Inspect when the first authoritative target appears; re-entering an active targeted contact also defaults to inspection. |
| **Local contact-comprehension metrics — complete in alpha.325** | Make private-alpha evidence easier to interpret | Record target locks, deliberate target inspections, encounter steps, and emergency-order timing in the existing local-only journal; summarize counts in tester-exported feedback without network telemetry. |
...
| **Structured outcome facts — complete in alpha.330** | Let observers compare the player's explanation with the actual run | Export the visible result explanation, replay guidance, installed-system durability and operating state, and surviving threats; show those facts beside tester words without scoring agreement. |
