...
LM-EA-2 through LM-EA-4 are complete through `0.3.0-alpha.353`; see [`lm_ea_2_cinder_spine_report.md`](lm_ea_2_cinder_spine_report.md), [`lm_ea_3_white_salt_report.md`](lm_ea_3_white_salt_report.md), and [`lm_ea_4_systems_report.md`](lm_ea_4_systems_report.md). The next implementation task is **LM-EA-5: event/meeting breadth, regional memory, replay goals, and composable endings**. Add consequences through the authoritative state and a bounded set of legible ending dimensions; do not substitute volume or generated prose for causal choices. Five consented, uncoached private-alpha sessions remain useful parallel calibration work; do not describe automated coverage as human comprehension evidence.
