...
**Current code baseline:** `0.3.0-alpha.344`
...
**Current merged baseline:** `0.3.0-alpha.344`, preserving the complete causal journey and private-alpha evidence workflow while making every human-review report create-only. Session and cohort summarizers now refuse existing destinations, their shared writer ships inside each checksummed cohort, and observer instructions keep automatic evidence separate from hand-written notes.
...
**Evidence support:** `0.3.0-alpha.344` protects human-authored evidence from replacement, `0.3.0-alpha.343` makes the public entry point match the playable slice, and the verified cohort tools establish artifact and state identity without inferring comprehension.
