...
All cues use the existing Interface Audio setting and remain silent when muted. Checkpoint-controlled buttons suppress the ordinary click so one successful command does not produce two competing confirmations. A family warning replaces only the generic contact-step cue at its deterministic warning step; it does not delay arrival, add a combat phase, or consume a random stream. Audio never advances state or controls timing.
