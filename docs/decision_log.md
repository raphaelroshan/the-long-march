...
This checkpoint deliberately reuses the established deterministic combat engine and route graph. A full-frame route planner, threat-to-module approach staging, roadside scenario tableau, and arrival presentation remain separate slices so their UI can be tested without weakening save, focus, and campaign correctness.
