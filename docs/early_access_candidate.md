...
**Candidate version:** `0.3.0-alpha.364`
...
The gate validates authored content, deterministic chapter runs, the complete save-version window, malformed-save rejection, explicit backup recovery, controller and responsive layouts, contrast and reduced motion, semantic audio, performance budgets, package manifests, rendered-frame visual evidence, the GPT56 execution contract, and the local-only playtest evidence workflow. Build & Local Data reports whether the primary and backup checkpoints are absent, valid, or unusable before a tester changes local files.
