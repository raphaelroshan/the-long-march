...
Generated builds are written to `build/` and are intentionally ignored by Git. Pull-request CI re-downloads its packaged Windows cohort and exercises the complete local evidence chain before merge. An owner-created `v*` tag builds and reverifies Windows and macOS cohorts, repeats that evidence smoke for both downloads, publishes standalone and cohort archives, and attaches `SHA256SUMS.txt` to the resulting GitHub prerelease. Manual workflow runs build candidates but do not publish a release.
